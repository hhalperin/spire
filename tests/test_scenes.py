...
SCENES_PATH = pathlib.Path(__file__).resolve().parent.parent / "content" / "scenes.json"
...
def test_every_client_screen_has_a_scene(scenes):
    """The client renders nine screens. None of them may fall through to nothing."""
    client_screens = {
        "title", "map", "room", "event", "reward", "campfire", "shop", "deck", "badges",
    }
    assert client_screens <= set(scenes["screens"])
