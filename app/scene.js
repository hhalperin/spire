...
export function composeScene({ scene, biome, modifier, rolls }) {
  ...
  const silhouettes = SCENES.layers.filter(
    (layer) => layer.kind === 'silhouette' && spec.layers.includes(layer.id),
  );

  /* Walk order: the whole base grammar first, layer by layer, then the
   * modifier's extras.
   *
   * This has to match `mapgen.scene_budget`, which sums the base grammar and
   * *appends* the worst modifier's extras. Splicing extras into their layer
   * instead pushed every later layer's slices along, so the same node drawn as
   * an elite read a different floor from the same node drawn plain — the two
   * disagreed about which float meant what. Extras belong at the tail, which is
   * exactly where the engine reserves room for them.
   *
   * Drawing order is unaffected: each path is appended to its own layer group,
   * and `mountScene` does the stacking. Only the dice are ordered here. */
  const walk = [];
  silhouettes.forEach((layer) => {
    (spec.grammar[layer.id] || []).forEach((entry) => walk.push({ layer, entry }));
  });
  const baseEntries = walk.length;
  silhouettes.forEach((layer) => {
    (((mod && mod.extra) || {})[layer.id] || []).forEach((entry) => walk.push({ layer, entry }));
  });
  ...
  // gradients cannot be siblings of SVG groups. mountScene() does the weaving;
  // this only has to keep the layers separable.
  const groupFor = (layer) => {
    if (!groups.has(layer.id)) {
      groups.set(layer.id, svgEl('svg', {
        viewBox: `0 0 ${W} ${H}`,
    ...
    return groups.get(layer.id);
  };

  let consumedBase = 0;
  walk.forEach(({ layer, entry }, index) => {
    if (index === baseEntries) consumedBase = cursor.i;
    /* A suppressed layer still costs its dice.
     *
     * The engine sizes the vector from the full grammar, because the budget is
     * a property of the scene and not of whichever modifier is applied. Skipping
     * a suppressed layer's slices left the cursor short and every layer after it
     * reading floats meant for something else — the reward scene uses `calm`,
     * which suppresses `near`, so it drew misaligned on every seed. A layer that
     * draws nothing is just an entry that draws nothing: it reserves its budget
     * and moves on. */
    const drawing = !suppressed.has(layer.id);

    // Fixed slice: reserve this entry's budget before drawing anything, so one
    // entry can never shift the dice of the next. The cost comes from the
    // component's own declaration, because only the builder knows how many
    // floats it draws.
    ...
    if (drawing && count > 0 && (entry.weight === undefined || appears <= entry.weight)) {
      ...
      for (let i = 0; i < count; i += 1) {
        // Each instance gets its own slot across the zone, so repeats read as a
        // colonnade rather than a pile.
  ...
  silhouettes.forEach((layer) => {
    ...
    if (group && group.childNodes.length) built.push({ id: layer.id, node: group });
  });

  if (baseEntries === walk.length) consumedBase = cursor.i;
  // `consumed` is the whole walk; `consumedBase` is where the modifier's extras
  // begin. Both are exported so tools/scene-consumption.mjs can check that the
  // extras sit at the tail, exactly where `mapgen.scene_budget` reserves them —
  // a length-only check passed while `elite` still spliced them mid-stream.
  return {
    layers: built, spec, biome: biomeSpec, modifier: mod,
    consumed: cursor.i, consumedBase,
  };
}
